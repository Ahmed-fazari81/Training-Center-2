...
import { LogOut, User as UserIcon, Home, Bell, Users, Settings, KeyRound, ListTodo, PlusCircle, Printer } from 'lucide-react';
...
export default function Layout() {
  ...
  const getNavLinks = () => {
    ...
    if (user.role === 'رئيس القسم') {
      return [
        ...
        { name: 'التقارير والطباعة', path: '/manager/report', icon: Printer },
      ];
    }
    return [
      { name: 'خططي', path: '/employee', icon: ListTodo },
      { name: 'إضافة خطة', path: '/employee/add', icon: PlusCircle },
    ];
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* Header */}
      <header className="bg-sky-700 text-white shadow-md sticky top-0 z-10 print:hidden">
      ...
      {/* Main Content Area */}
      <div className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 w-full flex flex-col sm:flex-row gap-6 print:block print:max-w-none print:p-0 print:m-0">

        {/* Sidebar/Nav for Desktop */}
        <nav className="hidden sm:flex flex-col w-64 bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden h-fit print:hidden">
        ...
        <nav className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around p-2 z-20 pb-safe print:hidden">
          {navLinks.map((link) => {
            const Icon = link.icon;
            const active = isActive(link.path);
            return (
      ...
      {/* السطران التعريفيان */}
      <footer className="text-center pb-24 sm:pb-4 pt-2 px-4 print:hidden">
        <p className="text-[11px] text-slate-400">فكرة راشد الزعابي مدرب تقنية معلومات</p>
        <p className="text-[11px] text-slate-400">تصميم وتنفيذ أحمد الفزاري</p>
      </footer>
    </div>
  );
}
